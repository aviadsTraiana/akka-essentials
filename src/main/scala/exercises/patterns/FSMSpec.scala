...
class FSMSpec extends TestKit(ActorSystem("FSMSpec"))
  ...
  //OneInstancePerTest make sure the states of the FSM does not mix between the tests
...
object FSMSpec {
    ...
    class VendingMachineFSM extends FSM[VendingState, VendingData] {
        ...
        onTransition { //when wanting side effects between transactions
            case stateA -> stateB => log.info(s"Transitioning from $stateA to $stateB")
        }

        initialize() //actually running the spec
    }
}
