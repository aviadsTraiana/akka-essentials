...
class SupervisionSpec
  ...
  "An all-for-one supervisor" should {
    "apply the all-for-one strategy" in {
      val supervisor =
        system.actorOf(Props[AllForOneSupervisor], "allForOneSupervisor")
      supervisor ! Props[FussyWordCounter]
      val child = expectMsgType[ActorRef]
...
object SupervisionSpec {
  ...
  class Supervisor extends Actor {
    override val supervisorStrategy: SupervisorStrategy = OneForOneStrategy() {
      ...
      case _: IllegalArgumentException => Stop //destroy internal state
  ...
  class AllForOneSupervisor extends Supervisor {
    //apply strategy for all children in case one fails
  ...
  class FussyWordCounter extends Actor {
    ...
    override def receive: Receive = {
      ...
      case s: String =>
        if (s.length > 20) throw new RuntimeException("too long")
        else if (!Character.isUpperCase(s(0)))
          throw new IllegalArgumentException("must start with uppercase")
        else words += s.split(" ").length
      case _ => throw new Exception("can receive only strings")
    }
  }
}
